...
function mostrarAgenda() {
    ...
    .then(data => {
        ...
        let agendaCompleta = [...agendaLocalStorage, ...contactosJson]; 

        agendaCompleta.forEach(function (contacto, index) {
            let contactoHTML = `<p><strong>Nombre</strong>: ${contacto.nombre}, <strong>Teléfono</strong>: ${contacto.telefono}`;
            if (contacto.imagen) {
                contactoHTML += ` <img src="${contacto.imagen}" alt="${contacto.nombre}" style="width: 50px; height: auto;">`;
            }
            if (index < agendaLocalStorage.length) {
                
                contactoHTML += ` <button class="editarBtn" data-index="${index}">Editar</button>`;
                contactoHTML += ` <button class="eliminarBtn" data-index="${index}">Eliminar</button>`;
            }
            contactoHTML += `</p>`;
            agendaHTML += contactoHTML;
        });
